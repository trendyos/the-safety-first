...
<div class="row wrapper border-bottom white-bg page-heading">
     ...
     <div class="hr-line-dashed"></div>
                                ...
                                <br>
                                <img src="{{asset('/'.$result->image)}}" height="50px;"/>
                                </div>
                                </div> 
						...
						<?php $counterQueAns = 0; ?>
							...
							<table class="table table-bordered questionSectionRow">
								...
								<tr class="panel panel-default delete_add_more_question_section_{{$value->id}}" rel="{{$value->id}}">
									...
									<td>	
									<?php $questionAnswers = DB::table('product_question_answers')->where('question_id',$value->id)->get(); ?>
										@if(!$questionAnswers->isEmpty())
							    @foreach($questionAnswers as $key => $answervalue)
										<div class="mws-form-item">
										    <div class="row">
										    <div class="col-sm-1">{{Form::checkbox("queans[$counterQueAns][answer][1][value]",1, $answervalue->answer == 1 ? true :false,['class'=>'','disabled'])}}</div>
										     <div class="col-sm-10">

										    {!! Form::text("queans[$counterQueAns][answer][1][option]",$answervalue->answer_option, ['class'=>'form-control' ,'disabled']) !!}
											<div class="error-message help-inline">
												<?php echo $errors->first('remark'); ?>
											</div>
											</div>
